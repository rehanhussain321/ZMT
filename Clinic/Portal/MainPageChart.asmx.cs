            ...
            try
            {
                ...
                if (ds != null)
                {
                    if (ds.Tables["MED_GetPharmasyUse"].Rows.Count > 0)
                    {
                        ...
                        {
                            PharmasyCountInfo.Add(new PharmasyCount { Item_Name = dr["Item_Name"].ToString(), Pharmasy_User = Convert.ToInt32(dr["Pharmasy_Use"]), Document_Date = dr["Document_Date"].ToString() });
                        }
                    }
                }
